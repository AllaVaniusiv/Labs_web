...
def get_printers():
    ...
    reverse_sort = request.args.get(
        "reverse_sort", type=lambda val: val.lower() == "true")
    ...
    if sort:
        final_printers = list(
            sorted(final_printers, key=sortingFunctions[sort]))

    if filter_option and not filter_option == "all":
        final_printers = list(
            filter(lambda a: a["category"] == filter_option, final_printers))
...
@app.route('/printers/printer/<int:printer_id>', methods=['GET'])
